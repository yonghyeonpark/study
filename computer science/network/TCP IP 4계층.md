...
- 과정
  1. 클라이언트가 `SYN` 플래그가 설정된 세그먼트를 전송하여 연결을 요청합니다.
  2. 서버는 해당 요청에 대해 `SYN-ACK` 플래그가 설정된 세그먼트로 응답합니다.
  3. 이후, 클라이언트가 `ACK` 플래그가 설정된 세그먼트를 전송함으로써 연결이 확립됩니다.

### 4-way handshake

- 데이터 손실 없이 안전하게 TCP 연결을 종료하기 위한 과정입니다.
- 과정
  1. 클라이언트가 `FIN` 플래그가 설정된 세그먼트를 전송합니다.
  2. 서버는 해당 요청에 대해 `ACK` 플래그가 설정된 세그먼트를 전송합니다.<br>(이때, 연결이 완전히 종료된 것이 아니며, 서버는 클라이언트에게 데이터를 계속 보낼 수 있습니다.)
  3. 서버도 데이터 전송을 종료할 준비가 되면, `FIN` 플래그가 설정된 세그먼트를 클라이언트에게 전송합니다.
  4. 이후, 클라이언트가 `ACK` 플래그가 설정된 세그먼트를 전송함으로써 연결이 완전히 종료됩니다.
